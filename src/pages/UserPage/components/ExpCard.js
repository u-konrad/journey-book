...
const ExpCard = ({ item, onAddFav, onRemoveFav, onDelete }) => {
    ...
    <Card id={item._id} className="shadow">
      ...
      <Card.Body className="mb-3 pt-0 px-4">
        ...
        <div className="d-flex justify-content-between mb-2">
          {!!item.category && (
            <CategoryLabel
              className="p-0 m-1 category-label"
        ...
        <Card.Text style={{ fontSize: "14px" }} className="mt-4 pe-5">
          {truncatedContent}
        </Card.Text>
      </Card.Body>
      ...
      <Card.Footer className="user-page__card-footer px-4">
        <div className="d-flex justify-content-between">
          ...
          <Button
            ...
            onClick={() => history.push(`/exps/${item._id}`)}
          >
           {t('card.details')}
          </Button>
          {token && (
